...
void codeForTask1(void *parameter)
{
  for (;;)
  {
    ...
    if (key == 'A')
    {
      ...
      Serial.print("before Waiting... ");
      Serial.println(passwordReceived);
    ...
    else if (key == 'B')
    {
      ...
      Serial.println("Finger detected!");
      int8_t match = getFingerprintID(); // Try reading a fingerprint

      if (match > 0)
      { // Check if a valid fingerprint ID is returned
